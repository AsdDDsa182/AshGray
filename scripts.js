...
async function loadCompaniesAndProducts() {
    try {
        ...
        // ✅ 여기에 한 번만 넣어야 함
        allProducts = []; // 혹시 모를 중복 방지를 위해 초기화
        ...
        initializeSearch(); // ✅ 검색 함수는 반드시 allProducts 채운 후 실행
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function () {
        ...
        // 기존 filteredProducts와 filteredCompanies를 합침
        const tempResults = [...filteredProducts, ...filteredCompanies];

        // 중복 제거: 상품 이름(name)과 회사(company)가 같은 것만 하나 남기기
        const uniqueResults = tempResults.filter((product, index, self) => 
            index === self.findIndex(p => 
                p.name === product.name && p.company === product.company
            )
        );

        // 정렬하고 표시
        const sortedResults = sortByRelevance(uniqueResults, normalize(rawSearch));

...
document.addEventListener('DOMContentLoaded', function() {
    ...
    }, 2100); // 로고와 로딩 바 애니메이션 완료 후 실행 5500
...
function toggleMobileMenu() {
    ...
    if (isOpen) {
        ...
        const bar1 = document.createElement('div'); bar1.classList.add('bottom-bar');
        const bar2 = document.createElement('div'); bar2.classList.add('bottom-bar');
...
// ───────────────────────────────────────────────────────────
// About Us 섹션이 화면에 나타날 때 애니메이션 실행 (Lazy-Load)
const aboutSection = document.getElementById('about-us');
if (aboutSection && typeof handleAboutUsAnimation === 'function') {
    const aboutObserver = new IntersectionObserver((entries, obs) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                handleAboutUsAnimation();   // 화면에 보이면 애니메이션 실행
                obs.unobserve(entry.target); // 한 번만 실행하고 관찰 해제
            }
        });
    }, { threshold: 0.2 });
...
// ───────── 타임라인 섹션 진입 후 GSAP/ScrollTrigger 초기화 ─────────
...
const handleScrollIndicators = throttle(() => {
    [
      { id: 'scroll-animation-section', idx: 1 },
      { id: 'best-services',            idx: 2 },
      { id: 'inquiry-intro',            idx: 3 },
    ].forEach(item => {
...
// 한 번에 한 번만 실행하도록, passive 옵션과 함께 등록

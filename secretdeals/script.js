(function(){
  ...
  ['quoteList','quoteListM'].forEach(id=>{ const root=document.getElementById(id); root.addEventListener('input', e=>{ if(e.target.matches('input[type=number]')) setQty(e.target.dataset.qid, +e.target.value); }); root.addEventListener('click', e=>{ const del=e.target.closest('[data-del]'); if(del){ delQuote(del.dataset.del); return; } const step=e.target.closest('[data-step]'); if(step){ const id=step.dataset.id, dir=parseInt(step.dataset.step,10); const it=quote.items.find(x=>x.id===id); if(it){ it.qty=Math.max(1,(it.qty||1)+dir); saveQuote(); } } }); });
  ...
  function goToQuotePage() {
    const quoteData = {
        items: quote.items,
        type: currentViewMode === 'rental' ? '렌탈' : '구매',
        duration: currentViewMode === 'rental' ? (currentRentalDuration === 'etc' ? '기간 협의' : `${currentRentalDuration}개월`) : ''
    };
    localStorage.setItem('quoteData', JSON.stringify(quoteData));
    window.location.href = 'quote.html';
  }

  submitQuoteBtn.addEventListener('click', goToQuotePage);
  submitQuoteMBtn.addEventListener('click', goToQuotePage);
  ...
  (function init(){
    renderBanner(); renderChannels(); updateQuoteUI();

(function(){
  ...
  async function handleFormSubmit(prefix, event) {
    ...
    // 인라인 폼 삭제로 인해 접두사 'inline'이 'f'로 대체되거나 사용되지 않도록 조정
    // 이 함수는 견적 폼(modalQuoteForm, 접두사 'f')만 처리하도록 수정됨
    if (prefix !== 'f') { return; } 
  ...
  (function init(){
    ...
    // 삭제된 부분: $('#inlineInquiryForm').addEventListener('submit', (e) => handleFormSubmit('inline', e));
  })();
})();